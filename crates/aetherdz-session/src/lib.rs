//! aetherdz-session — JSONL sessions with auto-title, recall, and ledger.
//!
...
use aetherdz_core::config::sessions_dir;
use aetherdz_core::error::{Error, Result};
use aetherdz_core::fs::{atomic_write, ensure_dir, safe_join};
...
mod tests {
    ...
    fn test_env(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("aetherdz-session-{tag}-{}", std::process::id()));
