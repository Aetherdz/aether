...
mod tests {
    ...
    fn safe_join_rejects_traversal() {
        let base = Path::new("/tmp/aetherdz-test");
    ...
    fn atomic_write_round_trips() {
        let dir = std::env::temp_dir().join(format!("aetherdz-fs-test-{}", std::process::id()));
    ...
    fn ensure_dir_is_idempotent() {
        let dir = std::env::temp_dir().join(format!("aetherdz-ensure-{}", std::process::id()));
