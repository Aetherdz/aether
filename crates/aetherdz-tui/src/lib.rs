//! aetherdz-tui — ratatui + crossterm terminal UI (Phase 4).
//!
//! Two screens:
...
pub struct RatatuiTui {
    screen: Screen,
    sessions: Vec<aetherdz_session::SessionSummary>,
    ...
    totals: aetherdz_session::Totals,
...
impl Default for RatatuiTui {
    fn default() -> Self {
        Self {
            ...
            palette_index: 0,
            totals: aetherdz_session::Totals::default(),
            last_error: None,
        }
    }
}
...
impl RatatuiTui {
    ...
    fn selected_session(&self) -> Option<&aetherdz_session::SessionSummary> {
    ...
    async fn send_turn(&mut self, question: String, model: &str) -> Result<(), TuiError> {
        ...
        if let Some(session) = &self.active {
            ...
            if let Some(u) = usage {
                ...
                session
                    .append_usage(aetherdz_session::SessionMeta {
                        turns: 1,
                        input_tokens: input,
...
mod tests {
    use super::*;
    use aetherdz_session::SessionId;
    use std::sync::Mutex;

    static ENV_LOCK: Mutex<()> = Mutex::new(());

    /// Point `AETHER_CONFIG_DIR` at a scratch dir so session/ledger reads
    /// never touch the developer's real store (same pattern as aetherdz-mcp).
    fn isolate(tag: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("aetherdz-tui-{tag}-{}", std::process::id()));
